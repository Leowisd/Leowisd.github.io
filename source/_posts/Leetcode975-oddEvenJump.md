---
title: Leetcode975-oddEvenJump
categories: leetcode
tags: [DP, Goolge]
description: Solution Report of LeetCode Acceptted
mathjax: true
date: 2020-03-03 13:50:43
permalink:
image:
---
...
class Solution {
    public int oddEvenJumps(int[] A) {
        ...
        map.put(A[n - 1], n - 1);
        for (int i = n - 2; i >= 0; i--){
            Map.Entry h = map.ceilingEntry(A[i]);
            Map.Entry l = map.floorEntry(A[i]);
            if (h != null) higher[i] = lower[(int)h.getValue()];
            if (l != null) lower[i] = higher[(int)l.getValue()];
            if (higher[i]) res++;
            map.put(A[i], i);
        }
